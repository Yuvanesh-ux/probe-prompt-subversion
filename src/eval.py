...
def evaluate_auroc_drop(probe_file: str, train_data_file: str, 
    ...
    # Prepare data with shape handling
    X_train = train_data['activations'].numpy()
    if len(X_train.shape) == 3:
        if X_train.shape[1] == 1:
            X_train = X_train.squeeze(1)  # Remove dimension of size 1
        else:
    ...
    if len(X_deploy.shape) == 3:
        if X_deploy.shape[1] == 1:
            X_deploy = X_deploy.squeeze(1)  # Remove dimension of size 1
        else:
    ...
    # Compute AUROCs with confidence intervals
    ...
    # Use Mann-Whitney U test on prediction scores between conditions
    ...
    results = {
        ...
        'auroc_drop_pp': float(auroc_drop * 100),  # Percentage points
